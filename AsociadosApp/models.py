...
class ResumenHistorico(models.Model):
    ...
    cierre = models.DateField(blank=True, null=True) # fecha en que se cerró el período para altas y bajas (5to día hábil período en curso)
    importe = models.DecimalField(max_digits=12, decimal_places=2, blank=True, null=True)
    facturado = models.DateField(blank=True, null=True) # fecha en que se procesó el cálculo y facturación (1er día hábil período siguiente) 
    empresa = models.ForeignKey('Empresa', on_delete=models.CASCADE, blank=True, null=True)
    created=models.DateTimeField(auto_now_add=True)
    updated=models.DateTimeField(auto_now_add=True)
